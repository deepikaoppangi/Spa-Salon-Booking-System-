...
app.config.update(
    ...
    MAIL_PORT='465',
    MAIL_USE_SSL=True,
    MAIL_USERNAME="deepikaoppangi@gmail.com",
    MAIL_PASSWORD="qjrk ikjc zmip ovjh"
)
mail = Mail(app)
    ...
    if request.method == "POST":
        ...
        if slot in allowed_slots and allowed_slots[slot]['start'] <= int(time.split(':')[0]) <= allowed_slots[slot]['end']:
            ...
            if existing_booking:
                ...
                # user_time = datetime(date.split(':')[0],date.split(':')[1], )
                

                # time_conflict = any(
                #     # abs(existing_book.time.seconds // 3600 - hour_part) < 1 and
                #     # abs(existing_book.time.seconds % 3600 // 60 - int(time.split(':')[1])) < 45
                #     abs((datetime(existing_book.time.hours, existing_book.time.minutes) - user_time).seconds //60) < 45
                ...
                existing_conflicts = Customers.query.filter(
                    Customers.date == date,
                    Customers.department == department,
                    Customers.slot == slot
                ).all()
                ...
                new_booking_dt = datetime(year, month, day, hour, minute)
                time_conflict = False
                for existing in existing_conflicts:
                    
                    #     # Parse existing booking's datetime
                    existing_date = existing.date  # Assuming this is a string in YYYY-MM-DD format
                    existing_time = existing.time  # Assuming this is a string in HH:MM format
                    
                    e_year, e_month, e_day = map(int, str(existing_date).split('-'))
                    e_hour, e_minute, e_second = map(int, str(existing_time).split(':'))
                    existing_dt = datetime(e_year, e_month, e_day, e_hour, e_minute, e_second)
                    
                    # Calculate time difference in minutes
                    delta_minutes = abs((new_booking_dt - existing_dt).total_seconds() / 60)
